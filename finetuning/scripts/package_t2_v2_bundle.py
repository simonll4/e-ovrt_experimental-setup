#!/usr/bin/env python3
"""Package the transportable T2 v2 bundle (full FT con optimizador explícito, D-FT-16).

Reemplaza al bundle de `1167864`: aquel usó `optimizer: auto`, ciego al conteo de
parámetros entrenables. Arranca desde el PESO BASE (no desde el best.pt de 1167864,
que resultó peor punto de partida). Lleva `train_t2.py` porque `train_t2_v2.py` reusa
de ahí el guard de alcance y la verificación open-vocabulary — el contrato es el MISMO.

Original (T2 v1, histórico):
...
VERSIONED_FILES = (
    "configs/t2_yoloe26s_full_v2.yaml",
...
def package(*, finetuning_root: Path, payload: Path, output: Path) -> dict[str, object]:
    ...
    try:
        ...
        metadata = {
            "schema_version": "eovrt.t2-v2-bundle.v1",
            "name": "t2_yoloe26s_full_v2",
            "amendment": "D-FT-16",
            "supersedes_run_id": "1167864",
            "tier": "T2",
            "decision": "D-FT-14/D-FT-15/D-FT-16",
            ...
            "trainer_config_sha256": sha256_file(
                finetuning_root / "configs" / "t2_yoloe26s_full_v2.yaml"
            ),
