...
function useRefreshAfterRun(id: string, running: boolean, refetch: () => Promise<unknown>) {
  const previous = useRef({ id, running })
  useEffect(() => {
    if (previous.current.id === id && previous.current.running && !running) void refetch()
    previous.current = { id, running }
  }, [id, running, refetch])
}

export function useExperimentAlerts(id: string, enabled: boolean, running = false) {
  const query = useQuery<ExperimentAlert[]>({
    queryKey: qk.experiments.alerts(id),
    queryFn: () => getExperimentAlerts(id),
    enabled: enabled && Boolean(id),
    refetchInterval: running ? POLL.enCurso : false,
  })
  useRefreshAfterRun(id, running, query.refetch)
  return query
}

export function useExperimentReport(id: string, enabled: boolean, running = false) {
  const query = useQuery<ExperimentReport>({
    queryKey: qk.experiments.report(id),
    queryFn: () => getExperimentReport(id),
    enabled: enabled && Boolean(id),
    // El 404 durante la ejecución no debe quedar como resultado final.
