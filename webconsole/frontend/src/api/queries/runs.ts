...
import type {
  ArtifactEntry, Composition, EvalResult, RunComparison, RunDetail, RunRow, TraceFrame,
...
export function useTraceIndex(runId: string, enabled: boolean, viva = false) {
  return useQuery<TraceIndex>({
    queryKey: qk.runs.traceIndex(runId),
...
export function useTracePage(
  ...
  return useQuery({
    queryKey: [...qk.runs.trace(runId, pagina), pageSize, solo],
...
export function useArtifacts(runId: string, enabled: boolean) {
  return useQuery<ArtifactEntry[]>({
    ...
    queryFn: async () => (await getArtifacts(runId)).items,
    enabled: enabled && Boolean(runId),
    staleTime: Infinity,
  })
}
